...
def load_config(config_path: str | Path | None = None) -> PipelineConfig:
    ...
    return PipelineConfig(
        razers3_path=config.get("mapping", "razers3", fallback="razers3"),
        mapping_threads=config.getint("mapping", "threads", fallback=4),
        solver=config.get("ilp", "solver", fallback="glpk"),
        ilp_threads=config.getint("ilp", "threads", fallback=1),
        delete_bam=config.getboolean("behavior", "deletebam", fallback=True),
        unpaired_weight=config.getfloat("behavior", "unpaired_weight", fallback=0.0),
        use_discordant=config.getboolean("behavior", "use_discordant", fallback=False),
    )
...
def run_pipeline(
    ...
    extension = "bam" if PYSAM_AVAILABLE else "sam"
    bam_paths = (
        input_files
        if bam_input
    ...
    representing_df = pd.DataFrame(
        [[a1, a2] for a1, a_l in representing.items() for a2 in a_l],
